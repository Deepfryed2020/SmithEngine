(function(){
  ...
  function unitOf(it){return String(it&&it.dimensionUnit||'cm').toLowerCase()==='mm'?'mm':'cm'}
  ...
  function install(){
    ...
    window.rowsForExport=function(list){
      ...
      const rows=list.map(it=>{
        ...
        vals.push(it.sourceIndex||'',it.id,it.sourceId||'',it.sku||'',it.barcode||'',it.description||'',it.location||'',it.zone||'',it.packType||'',pack,key,u.toUpperCase(),it.length||'',it.width||'',it.height||'',neat(cm(it.length,u)),neat(cm(it.width,u)),neat(cm(it.height,u)),it.weight||'',it.weight||'',it.netWeight||'',it.quantity||'',s,it.method||'',it.confidence||'',it.measuredAt||'',it.updatedAt||'',m.product?'YES':'NO',m.unit?'YES':'NO',m.box?'YES':'NO',m.box?'YES':'NO',it.notes||'');
        return vals;
      });
